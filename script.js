document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DOM ELEMENTS (with new additions) ---
    ...
    // --- Core Functions (updateCanvasTransform, Pan/Zoom, etc. are unchanged) ---
    ...
    // --- NEW: DOWNLOAD/SAVE LOGIC ---
    function serializeCanvas() {
        const items = canvas.querySelectorAll('.item');
        const data = Array.from(items).map(item => {
            const itemData = {
                id: item.id,
                type: item.classList.contains('sticky') ? 'sticky' : 'table',
    ...
    // --- NEW: UPLOAD/LOAD LOGIC ---
    function deserializeCanvas(jsonString) {
        try {
            ...
            data.forEach(itemData => {
                const newItem = createItem(itemData.type, itemData.left, itemData.top, itemData.width, itemData.height);
                
                // Restore content
                if (itemData.type === 'sticky') {
                    newItem.querySelector('textarea').value = itemData.content;
                } else if (itemData.type === 'table') {
                    const table = newItem.querySelector('table');
                    const thead = table.tHead;
                    const tbody = table.tBodies[0];

                    // Clear default table content
                    thead.innerHTML = '';
                    tbody.innerHTML = '';

                    // Rebuild headers
                    const headerRow = thead.insertRow();
                    itemData.tableData.headers.forEach(headerText => {
                    ...
                    // Rebuild rows
                    itemData.tableData.rows.forEach(rowData => {
                ...
                // Update next item ID to avoid collisions
                const idNum = parseInt(itemData.id.split('-')[1], 10);
                if (idNum > maxId) {
                    maxId = idNum;
                }
            });
            state.nextItemId = maxId + 1;
            // Reset view to make sure loaded content is visible
            resetViewBtn.click();
    ...
    function handleUpload(event) {
        ...
        reader.onload = (e) => {
            ...
            deserializeCanvas(content);
        };
        reader.readAsText(file);
        // Clear the input so the 'change' event fires again for the same file
    ...
    // --- NEW: Hook up new buttons ---
    downloadBtn.addEventListener('click', handleDownload);
    uploadBtn.addEventListener('click', () => fileInput.click()); // Trigger hidden file input
